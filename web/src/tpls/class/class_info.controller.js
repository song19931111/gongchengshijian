...
(function () {
    ...
    angular.module('mainApp')
        .controller('classCtrl',['$scope','classAjaxService','$uibModal','$state','LocalStorageServices',
            function ($scope,classAjaxService,$uibModal,$state,LocalStorageServices) {
                var GetAllUser = function () {
                    ...
                    //获取所有的用户
                    classAjaxService.getList($scope.currentPage).then(function (result) {
                        console.log("aaaaa"+$scope.classList);
                        $scope.paginationConf.totalItems = result.data.total;
                        $scope.classList = result.data.classList;
                    }).catch(function (data) {
                        console.log('error');
                    });
                ...
                ///////////////////////////////////////布置作业//////////////////////////////////////////////
                $scope.work = function (item) {
                    console.log("布置作业"+item);
                ...
                $scope.task = function (size,item) {
                    //这里很关键,是打开模态框的过程
                    var modalInstance = $uibModal.open({
                        animation: $scope.animationsEnabled,//打开时的动画开关
                        templateUrl: 'tpls/class/worktask.html',//模态框的页面内容,这里的url是可以自己定义的,也就意味着什么都可以写
                ...
                $scope.open = function (size,item) {
                    ...
                    var modalInstance = $uibModal.open({
                        animation: $scope.animationsEnabled,//打开时的动画开关
                        templateUrl: 'tpls/class/addclasscontent.html',//模态框的页面内容,这里的url是可以自己定义的,也就意味着什么都可以写
...
//这是模态框的控制器,记住$uibModalInstance这个是用来调用函数将模态框内的数据传到外层控制器中的,items则上面所说的入参函数,它可以获取到外层主控制器的参数
            ...
            if($scope.item == 'none'){
                $scope.item = {"username":"","classname":'',"classmissnum":"0","classchecknum":"0","answernum":"0",
                    "workscroe":"0","type":"add"};
            }else{
        ...
        .controller('ModalInstanceCtrl2', function ($scope, $uibModalInstance, items) {
//这是模态框的控制器,记住$uibModalInstance这个是用来调用函数将模态框内的数据传到外层控制器中的,items则上面所说的入参函数,它可以获取到外层主控制器的参数
        $scope.item = items;//这里就可以去外层主控制器的数据了
        if($scope.item == 'none'){
            $scope.item = {"workdate":"","content":"","type":"work"};
        }else{
            $scope.item['type'] ="modify";
        }
        ...
        .factory('authService', authService2);
    function authService($http, $log, $q, $localStorage, PermissionStore, ENV) {}
    function authService2($http, $log, $q, $localStorage, PermissionStore, ENV) {}
